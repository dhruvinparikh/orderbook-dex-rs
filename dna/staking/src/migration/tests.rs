...
fn upgrade_works() {
    ExtBuilder::default().build().execute_with(|| {
        ...
        // Check migration
        assert_eq!(
            <Staking as Store>::ErasStartSessionIndex::get(3).unwrap(),
            5
        );
        assert_eq!(
        ...
        assert_eq!(
            <Staking as Store>::ActiveEra::get().unwrap().start,
            Some(777)
        );
    ...
    for data_set in data_sets.iter() {
        ...
        ext.execute_with(|| {
            ...
            let old_staker_0_exposure = get_storage_value::<Exposure<AccountId, Balance>>(
                b"Staking",
                b"Stakers",
                &blake2_256(&old_staker_0.encode()),
            )
            .unwrap();
            let old_staker_1_exposure = get_storage_value::<Exposure<AccountId, Balance>>(
                b"Staking",
                b"Stakers",
                &blake2_256(&old_staker_1.encode()),
            )
            .unwrap();
            let (old_era_points_earned_total, old_era_points_earned_individual) =
            ...
            assert!(
                <Staking as Store>::ErasStakersClipped::iter().all(|exposure_clipped| {
