...
use frame_support::{
    debug, decl_error, decl_event, decl_module, decl_storage, traits::Get, Parameter,
};
...
impl<BlockNumber: sp_std::fmt::Debug> sp_std::fmt::Debug for OffchainErr<BlockNumber> {
    fn fmt(&self, fmt: &mut sp_std::fmt::Formatter) -> sp_std::fmt::Result {
        match *self {
            OffchainErr::TooEarly(ref block) => write!(
                fmt,
                "Too early to send heartbeat, next expected at {:?}",
                block
            ),
            OffchainErr::WaitingForInclusion(ref block) => write!(
                fmt,
                "Heartbeat already sent at {:?}. Waiting for inclusion.",
                block
            ),
...
impl<T: Trait> pallet_session::OneSessionHandler<T::AccountId> for Module<T> {
    ...
    fn on_before_session_ending() {
        ...
        debug::info!("---List of Validators from previous session: ");
